...
//! - envs = `<P>_HOST` not starting with `BASTION_`, sorted unique; listen
//!   port = 6033 + index in that sorted order
//! - credential = inline `<P>_USER` (pw `secrets/<p_lc>_password`) OR
//!   `<P>_CREDENTIALS=<K>` → `<K>_USER` (pw `secrets/<k_lc>_password`)
...
fn read_secret_file(secrets: &Path, name: &str) -> Result<String> {
    ...
    let s = std::fs::read_to_string(&p).with_context(|| format!("read secret {}", p.display()))?;
...
fn fingerprints_from_known_hosts(text: &str) -> (Vec<HostKeyFingerprint>, Vec<String>) {
    ...
    for (i, raw) in text.lines().enumerate() {
        ...
        let (_hosts, algo, blob) = match (it.next(), it.next(), it.next()) {
            ...
            _ => {
                warns.push(format!("known_hosts line {} malformed; skipped", i + 1));
                continue;
            }
        };
...
pub fn build_from_dir(src_dir: &Path) -> Result<(Config, ImportReport)> {
    ...
    // --- bastions ---
    let mut bastion_names_up: Vec<String> = vars
        .keys()
        .filter_map(|k| {
            k.strip_prefix("BASTION_")
                .and_then(|r| r.strip_suffix("_HOST"))
        })
        .map(|s| s.to_string())
        .collect();
    ...
    for up in &bastion_names_up {
        ...
        let host = vars
            .get(&format!("BASTION_{up}_HOST"))
            .ok_or_else(|| anyhow!("BASTION_{up}_HOST missing"))?
            .clone();
        let port: u16 = vars
            .get(&format!("BASTION_{up}_PORT"))
            .map(|s| s.parse())
            .transpose()
            .ok()
            .flatten()
            .unwrap_or(22);
        let ssh_user = vars
            .get(&format!("BASTION_{up}_USER"))
            .cloned()
            .unwrap_or_default();
        let auth_kind = vars
            .get(&format!("BASTION_{up}_SSH_AUTH"))
            .map(|s| s.as_str())
            .unwrap_or("password");
        ...
        let auth = if auth_kind == "key" {
            ...
            BastionAuth::Key {
                private_key_pem: SecretBytes::new(pem),
                passphrase: None,
            }
        } else {
            let pw = read_secret_file(&secrets, &format!("bastion_{lc}_password"))?;
            BastionAuth::Password {
                password: SecretStr::new(pw),
            }
        };

        let pinned =
            match std::fs::read_to_string(secrets.join(format!("bastion_{lc}_known_hosts"))) {
                Ok(kh) => {
    ...
    // --- envs ---
    let mut env_names_up: Vec<String> = vars
        .keys()
    ...
    if env_names_up.is_empty() {
        bail!(
            "no envs found in {} (need at least one <NAME>_HOST)",
            env_path.display()
        );
    }
    ...
    for (idx, env_up) in env_names_up.iter().enumerate() {
        ...
        let (cred_name, backend_user, pw_secret) =
            if let Some(u) = vars.get(&format!("{env_up}_USER")) {
                ...
                let key = vars.get(&format!("{env_up}_CREDENTIALS")).ok_or_else(|| {
                    anyhow!("env {env_lc}: no inline {env_up}_USER and no {env_up}_CREDENTIALS")
                })?;
                let key_up = key.to_uppercase();
                let key_lc = key.to_lowercase();
                let u = vars.get(&format!("{key_up}_USER")).ok_or_else(|| {
                    anyhow!("env {env_lc}: {env_up}_CREDENTIALS={key} but {key_up}_USER unset")
                })?;
        ...
        let max_conn = vars
            .get(&format!("{env_up}_MAX_CONNECTIONS"))
            .or_else(|| {
    ...
    cfg.validate()
        .context("imported config failed validation")?;
    Ok((cfg, report))
}
