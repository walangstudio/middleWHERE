...
fn parse_handshake_response(buf: &[u8]) -> Result<ParsedHandshakeResponse<'_>, HandshakeError> {
    ...
    if client_caps & CLIENT_SECURE_CONNECTION != 0 {
        ...
        if cur >= buf.len() { return Err(Malformed("missing auth-response length")); }
        let n = buf[cur] as usize; cur += 1;
        if cur + n > buf.len() { return Err(Malformed("auth-response truncated")); }
        auth_response = &buf[cur..cur + n];
        cur += n;
    } else {
        // legacy: NUL-terminated
        let (ar, n) = read_nul_string(&buf[cur..]).ok_or(Malformed("missing legacy auth-response NUL"))?;
        auth_response = ar;
        cur += n;
    }
    ...
    Ok(ParsedHandshakeResponse { client_caps, username, auth_response, database, auth_plugin })
}
    ...
    S: AsyncRead + AsyncWrite + Unpin,
{
    ...
    let parsed = match parse_handshake_response(&response) {
        Ok(p) => p,
        Err(e) => {
            let _ = write_packet(stream, &mut seq,
                &build_err_packet(ER_HANDSHAKE_ERROR, b"08S01", "bad handshake")).await;
            return Err(e);
        }
    };
    ...
    let native_response: Vec<u8> = if needs_switch {
        write_packet(stream, &mut seq,
            &build_auth_switch_request(&scramble)).await?;
    ...
    let user_ok = bool::from(
        username_seen.as_bytes().ct_eq(expected_env_name.as_bytes()),
    );
    ...
    if !allowed {
        ...
        let _ = write_packet(stream, &mut seq,
            &build_err_packet(ER_ACCESS_DENIED_ERROR, b"28000", "access denied")).await;
        return Err(HandshakeError::AuthFailed);
    }

    let database = parsed.database.map(|d| String::from_utf8_lossy(d).to_string());
...
pub mod testing {
    ...
    pub fn build_handshake_response(
        ...
        if database.is_some() { caps |= CLIENT_CONNECT_WITH_DB; }
    ...
    pub fn err_code_of(packet: &[u8]) -> Option<u16> {
        if packet.first() != Some(&0xFF) || packet.len() < 3 { return None; }
...
mod golden {
    ...
    fn initial_handshake_bytes_frozen() {
        ...
        let expected: &[u8] = &[
            10, // protocol version
            // "8.4.0-mdlwhere\0"
            0x38, 0x2e, 0x34, 0x2e, 0x30, 0x2d, 0x6d, 0x64,
            0x6c, 0x77, 0x68, 0x65, 0x72, 0x65, 0x00,
            0x44, 0x33, 0x22, 0x11, // thread id LE
            0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, // scramble[..8]
            0x00, // filler
            0x08, 0x82, // caps low (PROTOCOL_41|SECURE|PLUGIN_AUTH|CONNECT_WITH_DB|DEPRECATE_EOF) & 0xFFFF
            0x21, // charset
            ...
            0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10,
            0x11, 0x12, 0x13, 0x14, // scramble[8..]
    ...
    fn ok_packet_bytes_frozen() {
        assert_eq!(build_ok_packet(), vec![0x00, 0x00, 0x00, 0x02, 0x00, 0x00, 0x00]);
    }
