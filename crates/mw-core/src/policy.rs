...
const MYSQL_DANGEROUS_FUNCS: &[&str] = &[
    "load_file", "sys_exec", "sys_eval", "sys_get",
    "lo_import", "lo_export",
];
...
const PG_SAFE_SET_VARS: &[&str] = &[
    "names", "client_encoding", "timezone", "datestyle",
    "application_name", "statement_timeout", "lock_timeout",
    "idle_in_transaction_session_timeout", "extra_float_digits",
    "bytea_output", "intervalstyle",
];
...
impl Decision {
    ...
    pub fn reason(&self) -> Option<&'static str> {
        match self { Decision::Allow => None, Decision::Deny(r) => Some(r) }
    }
}
...
pub fn evaluate(sql: &str, policy: &Policy, profile: &PolicyProfile) -> Decision {
    ...
    let (allow_dml, allow_ddl, allow_admin) = match policy {
        ...
        Policy::Custom { allow_dml, allow_ddl, allow_admin } =>
...
fn check_statement(
    ...
    match stmt {
        ...
        ShowCreate { .. } | ShowColumns { .. } | ShowTables { .. }
        | ShowFunctions { .. } | ShowStatus { .. } | ShowVariables { .. }
        | ShowVariable { .. } | ShowDatabases { .. } | ShowSchemas { .. }
        | ShowCollation { .. } => Decision::Allow,
        Use { .. } => Decision::Allow,
        SetNames { .. } | SetNamesDefault { .. } => Decision::Allow,
        SetVariable { local, hivevar, variables, .. } =>
            check_set(*local, *hivevar, variables, profile.safe_set_vars),

        // --- gated kinds ---
        Insert { .. } | Update { .. } | Delete { .. }
            if allow_dml => Decision::Allow,
        Insert { .. } | Update { .. } | Delete { .. }
            => Decision::Deny("DML not allowed by policy"),

        Truncate { .. } | Drop { .. } | CreateTable { .. } | CreateView { .. }
        | CreateIndex { .. } | CreateSchema { .. } | CreateDatabase { .. }
        | AlterTable { .. } | AlterIndex { .. } | AlterView { .. }
            if allow_ddl => Decision::Allow,
        Truncate { .. } | Drop { .. } | CreateTable { .. } | CreateView { .. }
        | CreateIndex { .. } | CreateSchema { .. } | CreateDatabase { .. }
        | AlterTable { .. } | AlterIndex { .. } | AlterView { .. }
            => Decision::Deny("DDL not allowed by policy"),

        Grant { .. } | Revoke { .. }
            if allow_admin => Decision::Allow,
        Grant { .. } | Revoke { .. }
            => Decision::Deny("admin not allowed by policy"),
...
fn check_query(q: &Query) -> Decision {
    ...
    if let Some(d) = scan_select_for_outfile(&q.body) { return d; }
    Decision::Allow
}
...
mod tests {
    ...
    fn deny(sql: &str) {
        ...
        assert!(matches!(d, Decision::Deny(_)), "expected Deny for {sql:?}, got {d:?}");
    }
    fn allow(sql: &str) {
        let d = evaluate(sql, &Policy::ReadOnly);
    ...
    #[test] fn select_allowed()        { allow("SELECT 1"); }
    #[test] fn select_with_where()     { allow("SELECT id FROM t WHERE x = 1"); }
    #[test] fn show_tables_allowed()   { allow("SHOW TABLES"); }
    #[test] fn use_db_allowed()        { allow("USE app"); }
    #[test] fn explain_select_allowed() { allow("EXPLAIN SELECT * FROM t"); }

    #[test] fn update_denied()         { deny("UPDATE t SET x=1"); }
    #[test] fn delete_denied()         { deny("DELETE FROM t"); }
    #[test] fn drop_denied()           { deny("DROP TABLE t"); }
    #[test] fn truncate_denied()       { deny("TRUNCATE TABLE t"); }
    #[test] fn alter_denied()          { deny("ALTER TABLE t ADD COLUMN c INT"); }
    #[test] fn grant_denied()          { deny("GRANT ALL ON *.* TO 'x'@'%'"); }
    #[test] fn create_table_denied()   { deny("CREATE TABLE t (x INT)"); }

    #[test] fn leading_comment_delete_denied() {
        deny("/* harmless */ DELETE FROM t");
    }
    #[test] fn line_comment_drop_denied() {
        deny("-- ok\nDROP TABLE t");
    }
    #[test] fn whitespace_delete_denied() {
        deny("   \n\t  DELETE FROM t");
    }
    #[test] fn stacked_statements_denied() {
        deny("SELECT 1; DELETE FROM t");
    }
    #[test] fn select_into_outfile_denied() {
        deny("SELECT * FROM t INTO OUTFILE '/tmp/x'");
    }
    #[test] fn load_file_denied() {
        deny("SELECT LOAD_FILE('/etc/passwd')");
    }

    #[test] fn readwrite_policy_allows_update() {
        assert_eq!(evaluate("UPDATE t SET x=1", &Policy::ReadWrite), Decision::Allow);
    }
    #[test] fn readwrite_still_denies_ddl() {
        assert!(matches!(evaluate("DROP TABLE t", &Policy::ReadWrite), Decision::Deny(_)));
    }

    #[test] fn unparseable_denied() {
    ...
    #[test] fn explain_hides_no_bypass() {
